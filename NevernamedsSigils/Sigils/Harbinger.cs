...
namespace NevernamedsSigils
{
    public class Harbinger : AbilityBehaviour
    {
        ...
        {
            if (base.Card.slot != null && (base.Card.slot.IsPlayerSlot == deathSlot.IsPlayerSlot) && fromCombat && card.slot != null && !card.InOpponentQueue)
            {
                return true;
            }
            else return false;
        ...
        public override IEnumerator OnOtherCardDie(PlayableCard card, CardSlot deathSlot, bool fromCombat, PlayableCard killer)
        {
            if (deathSlot.Card == null || deathSlot.Card.Dead)
            {
                ...
                if (base.Card.Info.name == "Nevernamed GiantEarthworm")
                {
                    yield return new WaitForSeconds(0.1f);
                    if (oldSlot && oldSlot.Card == null)
                    {
                        CardInfo segment = CardLoader.GetCardByName("Nevernamed EarthwormSegment");
                        segment.mods.Add(base.Card.CondenseMods());
