    ...
    public class HaunterCustom : AbilityBehaviour
    {
        public static void Init()
        {
            AbilityInfo newSigil = SigilSetupUtility.MakeNewSigil("Haunter", "When [creature] dies to combat, it haunts the space it died in. Creatures that enter this space absorb its old sigils.",
                      ...
                      triggerText: "[creature] leaves its sigils behind in the ethereal plane!");
        ...
        public override IEnumerator OnPreDeathAnimation(bool wasSacrifice)
        {
            ...
            {
                if (Card.GetComponentInChildren<PixelCardAbilityIcons>() == null) { yield break; }
                foreach (GameObject obj in Card.GetComponentInChildren<PixelCardAbilityIcons>().abilityIconGroups)
                {
                    if (obj.activeSelf)
                    {
                        foreach (SpriteRenderer icon in obj.GetComponentsInChildren<SpriteRenderer>())
                        {
                            GameObject sigils = UnityEngine.Object.Instantiate(icon.transform.gameObject, (icon.transform.position + new Vector3(0, 0f, 0)), icon.transform.rotation);
                            foreach (Renderer rend in sigils.GetComponentsInChildren<Renderer>())
                            {
                                rend.enabled = true;
                                rend.material = new Material(rend.material);
                            }
                            if (sigils.GetComponent<SpriteRenderer>())
                            {
                                SpriteRenderer rend = sigils.GetComponent<SpriteRenderer>();
                            }
                            Texture tex = sigils.GetComponent<Renderer>().material.mainTexture;
                            sigils.GetComponent<Renderer>().material.mainTexture = tex;
                            sigils.GetComponent<Renderer>().material.SetColor("_Color", GameColors.Instance.brightGold);
            ...
            foreach (GameObject floater in hauntSigils)
            {
                if (floater.GetComponent<HauntedSigilFloatData>() != null)
                {
                    ...
                    if (floatData.MovingUp)
                    {
                        ...
                        if (Tools.GetActAsInt() == 2) { movement *= 0.5f; }
                ...
                else
                {
                    ...
                    floatData.lowerLimit = floater.transform.position.y - (Tools.GetActAsInt() == 2 ? 0.045f : 0.09f);
                    floatData.upperLimit = floater.transform.position.y + (Tools.GetActAsInt() == 2 ? 0.08f : 0.16f);
                    floatData.lowerSlowThreshold = floater.transform.position.y - (Tools.GetActAsInt() == 2 ? 0.025f : 0.065f);
                    floatData.upperSlowTheshold = floater.transform.position.y + (Tools.GetActAsInt() == 2 ? 0.07f : 0.135f);

                    //Random Start
                    float randomMod = Tools.GetActAsInt() == 2 ? UnityEngine.Random.Range(-0.045f, 0.08f) : UnityEngine.Random.Range(-0.09f, 0.16f);
